...
export default function HostelCard({ hostel, onBookInspection }: HostelCardProps) {
  ...
  return (
    <Card className="overflow-hidden card-hover cursor-pointer border-2 border-border/50 shadow-lg" data-testid={`card-hostel-${hostel.id}`}>
      <img 
      ...
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <h3 className="text-xl font-bold text-foreground leading-tight" data-testid={`text-hostel-title-${hostel.id}`}>
            {hostel.title}
          </h3>
          <Badge 
            variant={hostel.availability ? "default" : "secondary"}
            className={hostel.availability 
        ...
        <div className="flex items-center text-muted-foreground mb-4">
          ...
          <span className="text-sm font-medium" data-testid={`text-location-${hostel.id}`}>
            {hostel.location?.name || 'Location not specified'}
          </span>
        </div>
        
        <p className="text-muted-foreground text-sm mb-6 leading-relaxed" data-testid={`text-description-${hostel.id}`}>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Button 
            ...
            className="bg-primary text-primary-foreground hover:bg-primary/90 px-6 py-2 rounded-lg font-semibold transition-all focus-ring shadow-md"
        ...
        {hostel.amenities && hostel.amenities.length > 0 && (
          <div className="flex items-center flex-wrap gap-4 text-sm text-muted-foreground" data-testid={`amenities-${hostel.id}`}>
            {hostel.amenities.slice(0, 3).map((amenity: string, index: number) => (
              <div key={index} className="flex items-center bg-secondary/50 px-3 py-1 rounded-lg" data-testid={`amenity-${hostel.id}-${index}`}>
                {getAmenityIcon(amenity)}
                <span className="ml-2 font-medium">{amenity}</span>
              </div>
            ))}
            {hostel.amenities.length > 3 && (
              <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-lg font-medium" data-testid={`amenities-more-${hostel.id}`}>
